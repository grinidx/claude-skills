...
2. Open it for full-resolution preview (`open <path>` on macOS, `xdg-open <path>` on Linux)
3. Report the cost
...
2. For clipboard: save it to a temp file first (`osascript` on macOS, `xclip -selection clipboard -t image/png -o > /tmp/clip.png` on Linux)
...
$PY $GEN --thinking medium --preset diagram "OAuth flow" out.png

# Seed for reproducibility
$PY $GEN --seed 42 "prompt" out.png
...
Requires `OPENAI_API_KEY` in the environment. Use `--provider openrouter` with `OPENROUTER_API_KEY` to route via OpenRouter.
